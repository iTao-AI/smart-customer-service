...
from smart_ai.agent.message_processor import (
    MessageProcessor,
    ProcessorConfig,
    MessageResponse,
)
from smart_ai.agent.actions import register_action, Action
from smart_ai.agent.graph import (
    get_message_processing_graph,
    create_initial_state,
)
from smart_ai.core.tracker import DialogueStateTracker
from smart_ai.core.domain import Domain
from smart_ai.core.stores import create_tracker_store, TrackerStore
from smart_ai.dialogue_understanding.flow import FlowsList, FlowLoader
from smart_ai.dialogue_understanding.generator import LLMCommandGenerator
from smart_ai.dialogue_understanding.processor import CommandProcessor
from smart_ai.policies import PolicyEnsemble, FlowPolicy, EnterpriseSearchPolicy
from smart_ai.shared.yaml_loader import read_yaml_file
from smart_ai.shared.config import SmartAIConfig, LLMConfig
...
class Agent:
    ...
    def load(
        ...
        if project_path.is_file() and project_path.name.endswith(".tar.gz"):
            ...
            # 解压到临时目录
            temp_dir = tempfile.mkdtemp(prefix="smrt_model_")
        ...
        from smart_ai.shared.config import EndpointsConfig
        ...
        if config_path.exists():
            ...
            if config_data:
                ...
                if "retrieval" in config_data:
                    from smart_ai.shared.config import RetrievalConfig
        ...
        if llm_config:
            from smart_ai.dialogue_understanding.generator import (
                LLMCommandGenerator,
                LLMGeneratorConfig,
            )
        ...
        # 创建策略
        from smart_ai.policies import EnterpriseSearchPolicyConfig
        ...
        if retriever_class_path:
            try:
                from smart_ai.retrieval import create_retriever
        ...
        if nlg_config and nlg_config.rephrase_enabled:
            try:
                from smart_ai.nlg import ResponseRephraser, RephraserConfig, TemplateNLG
                ...
                if rephrase_llm_config:
                    ...
                    # 创建LLM客户端
                    from smart_ai.shared.llm import create_llm_client
