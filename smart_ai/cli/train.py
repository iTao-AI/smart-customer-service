...
def train_command(
    ...
    示例:
        smrt train
        smrt train --data ./my_data --output ./my_models
        smrt train --dry-run  # 仅验证
    """
    verbose = ctx.obj.get("verbose", False)
    debug = ctx.obj.get("debug", False)
    
    click.echo("=" * 50)
    click.echo("SmartAI AI - 模型训练")
    click.echo("=" * 50)
    ...
    try:
        ...
        from smart_ai.shared.config import SmartAIConfig
        
        # 加载配置（用于验证）
        click.echo("加载配置...")
        smrt_config = SmartAIConfig.load(str(config_path))
        click.echo("  配置加载成功")
        ...
        if dry_run:
            ...
            from smart_ai.dialogue_understanding.flow import FlowLoader
