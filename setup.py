...
- smart_ai/ 是我们要安装的包（里面有 __init__.py）
- 所以 setup.py 放在 smart_ai/ 的父目录，即项目根目录
...
2. smart_ai 包会被注册到 Python 环境，可以在任何地方 import
3. smrt 命令会被注册到系统，可以在命令行直接使用：
   - smrt init    # 初始化项目
   - smrt train   # 训练模型
   - smrt run     # 启动服务
   - smrt inspect # 交互式测试
...
Q: 安装后怎么卸载？
A: pip uninstall smart_ai

Q: 怎么查看是否安装成功？
A: pip show smart_ai  或者  python -c "import smart_ai; print(smart_ai.__version__)"
"""
...
def read_version() -> str:
    """从 smart_ai/__init__.py 读取版本号。
    
    为什么版本号定义在 __init__.py 而不是这里？
    - 这样代码里可以通过 smart_ai.__version__ 获取版本
    - 版本号只需要维护一处，不会出现不一致
    """
    init_path = Path(__file__).parent / "smart_ai" / "__init__.py"
...
setup(
    ...
    # 简短描述（一句话）
    description="LLM驱动的对话系统 - 基于 LLM 驱动的智能对话架构精简实现",
    ...
    # 
    # include: 只包含这些包（smart_ai 和它的所有子包）
    ...
    # 命令行入口点（CLI）
    # ------------------------------------------------------------------------
    # 安装后会创建 smrt 命令，执行时调用 smart_ai.cli 模块的 main 函数
    # 
    # 格式："命令名=模块路径:函数名"
    # 安装后可以直接在终端运行：smrt init / smrt train / smrt run
    entry_points={
        "console_scripts": [
            "smrt=smart_ai.cli:main",
        ],
    },
